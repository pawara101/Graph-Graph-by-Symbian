...
def process_epoch(phase, _epoch, p, _dataloader, _model, _optim=None):
    ...
    if (phase == 'train'):
        ...
        _model.eval()
        state_dict = torch.load("model_checkpoints/dad/SpaceTempGoG_detr_dad_6.pth")
    ...
    for iter, _data in enumerate(_dataloader):
        ...
        loss, logits = _model.forward(feat_rgb.cuda(), [label_cause.cuda(), label_effect.cuda()])
...
def compute_exact_overlap(logits, cause_gt, effect_gt, pred_type='both'):
    ...
    def _count_iou(pred_label, _cls, cls_gt):
        ...
        union = torch.sum((pred_label == _cls).float(), dim=1, keepdim=False) + (cls_gt[:, 1] - cls_gt[:, 0]) * float(
            T) - inter
...
def iterate_epoch(p, dataloader, model, optimizer=None):
    ...
    for i_batch, v in enumerate(dataloader):
        ...
        if ('Segmentation' in p['architecture_type']):
            if 'MSTCN' in p['architecture_type']:
                ...
                for logits in _logits:
                    ...
                    loss2 += torch.mean(torch.clamp(F.mse_loss(F.log_softmax(logits[:, :, 1:], dim=1),
                                                               F.log_softmax(logits.detach()[:, :, :-1], dim=1),
                                                               reduction="none"), min=0,
                                                    max=p['mse_tau'] * p['mse_tau']))
            else:
                logits = model(rgb, flow)
                loss1 = F.cross_entropy(logits, causality_mask, reduction='mean')
                loss2 = torch.mean(torch.clamp(
                    F.mse_loss(F.log_softmax(logits[:, :, 1:], dim=1), F.log_softmax(logits.detach()[:, :, :-1], dim=1),
                               reduction="none"), min=0, max=p['mse_tau'] * p['mse_tau']))
        ...
        # accumulate tIoU
        if ('Segmentation' in p['architecture_type']):
            cause_iou, effect_iou = compute_exact_overlap(logits.cpu(), cause_reg.cpu(), effect_reg.cpu(),
                                                          p['prediction_type'])
